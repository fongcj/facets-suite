...
qc <- function(maf, facets, igv=F){
  ...
  for (s in samples){
    ...
    tryCatch({
      ...
      frac_subcl_cna <- facets.fit[cf < purity, sum(end - start)] / facets.fit[, sum(end - start)]
      ...
      if(igv){
        igv.adj <- rbind(igv.adj, center_igv_file(out))
      }
  ...
  write.table(flagged, file="FACETS_QC_summary.txt", quote=F, row.names=F, col.names=T,
              sep="\t")
  write.table(igv.adj, file="igv.adj.seg", quote=F, row.names=F, col.names=T,
              sep="\t")
